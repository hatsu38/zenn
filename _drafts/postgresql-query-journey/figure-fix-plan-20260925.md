...
- 背景は `<rect width="640" height="H" fill="#f5f7f9"/>`（問いの型は、試作の `01-search-window`・`05-growing-library` と同じクリーム色 `#fffdf7`）。見出しは `<text x="24" y="48" class="h">`。札（「模型」「実測」など）は右上に置き、右端を x=616 にそろえる。見出しと同じ行に置くときは、見出しとの間を12以上あける（入らないときは次の行の右端へ下げる）。「模型」の札は幅68（x=548）にする。
...
**数値と出典:** 本文20〜24行目の表（記録の本番号 2・1・2、本1＝星の図鑑、本2＝海の図鑑）。

**描く（問いの型。人物は描かない）:**
- 見出し：`本の番号だけでは、題名が分からない`（今のまま）
- 札は付けない。
- 左（または上）：画面の枠「読了記録」。中に3行「本番号 2｜題名 ？」「本番号 1｜題名 ？」「本番号 2｜題名 ？」。「最新」「…20件」は付けない（本文85行目は最新20件がすべて別の本だったと書くので、本2が2回ある3行を最新20件の一部に見せない。3行は直後の「3件の記録」の例と同じ）。
- 右（または下）：画面の外に表の枠「本の表 books（100万冊）」（`.panel`。ページ枠 `.page` は1ページを表すので使わない）。中に行カード「1　星の図鑑」「2　海の図鑑」と「⋮」。
...
pairs = [
    ('![本の番号だけでは、題名が分からない](/images/postgresql-query-journey/09-title-lookup.png)',
     '![読了記録には本番号2・1・2しかなく、題名は画面の外の本の表で番号の行を探して付ける](/images/postgresql-query-journey/09-title-lookup.png)'),
    ('*学ぶきっかけを描く、説明用の場面。*',
...
計画のまま進めて問題が出たところと、レビューのあとで直したところ。

| 場面 | 図・道具 | 計画のままで出た問題 | 直し方 |
| --- | --- | --- | --- |
| 実行の段取り | Task 2〜9 | エージェントの worktree の隔離は、作業ブランチではなく GitHub の main（#15 のマージ後）から worktree を作ったため、Task 1 の検査がなかった | 作業ブランチの Task 1 のコミットから worktree を8つ作り、そこで作業してもらった |
| 事前の点検 | `03-row-width`・`11-visibility-map` | 見出しの右端（約570・約592）が右上の札とぶつかる | `03-row-width` は札を1段目の右端へ下げ、`11-visibility-map` は見出しを焦点に合わせて変えた |
| タスクのレビュー | `03-pages-and-rows` | 焦点のラベルに茶色（要求の色）を使った | 紺の太字にし、枠の名前と離した |
| タスクのレビュー | `09-title-lookup` | 表全体にページ枠を使った。背景が問いの型のクリーム色でなかった（計画の全体の約束から抜けていた）。本2が2回ある3行が、本文の「最新20件はすべて別の本」と食い違って見えた | 表は `.panel`、背景は `#fffdf7` にし、「最新」「…20件」を外した。キャプションと矢印の始点の食い違いも直した |
| 統合 | `09-hash-join` | Task 4 が作業領域と値カードの部品を足す前に描いたため、箱が計画のノード、数字が行カードの形だった。ラベルの「→」が「だから」と読めた | ①②とも作業領域の枠と値カードにそろえ、ラベルを「余りが2なので、箱2だけを見る」にした |
| 統合 | `11-snapshots` | 見出し「読む時点によって、見える版が変わる」が、②（確定後に読んでも版1）と逆に聞こえた | 見出しを「Bが確定しても、Aは版1を読む」にした |
| 統合 | `04-memory-regions`・`11-visibility-map`・`03-row-width` | 凡例と矢印のラベルが同じ基線、×のマスが窮屈、✓のマスが Index の枠と同じ見た目、原本の書き方の乱れ | 基線をずらし、マスを幅120にし、✓のマスを `.panel` にし、札と線の色をクラスにそろえた |
| Task 10 | book-flow.html | 計画は全部の図の見出しを catalog に合わせる手順だったが、ほかの章の図のラベルは説明文だった | この段階で直した8枚だけを合わせた |
| 全体レビュー | 検証ログの台帳・設計書・README | 新しい実測ログが台帳に載っていなかった。部品見本にページの形が2種類あった。描画の検査は transform を測れない | 台帳に節を足し、設計書4.1にページの2つの描き方を書き、README と設計書4.4に transform を使わないことを書いた |

横展開の次の段階に活かすこと：

- worktree は作業ブランチのコミットから作る（エージェントの隔離に任せない）。報告を本体の `.superpowers/sdd/` へ書けるよう、実装担当は隔離せずに動かす。
- 部品見本に部品を足すタスクと、その部品を使う図のタスクを並行させると、見た目がずれる。部品を足すタスクを先に終える。
- 図の中で新しく出す数（例：10,101枚）は、本文の仮定から計算できるものだけにし、キャプションか前後の文でも触れる。
- 原本では `transform` を使わない。
