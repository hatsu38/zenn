...
**描く（Task 6 で承認された `06-btree-path.svg` の木を、位置・大きさ・部品を変えずに写す。第3章の約束は同じ）:**
- 見出し：`4以上8以下なら、隣の箱も開く`。
- 上：`06-btree-path` と同じ案内板（`.route`、「境目　4｜7」）と三つの箱、同じ枝。
  - 案内板から真ん中の箱へ `.ref` の矢印。ラベル「① 4がある箱へ」。
  - 左の箱：`.ghost` と×、「開かない」。
  - 真ん中の箱（`.route`）：4・5・6 に ✓。
  - 真ん中の箱から右の箱へ `.ref` の矢印。ラベル「② 隣の箱へ」。
  - 右の箱（`.route`）：7・8 に ✓、9 に ×、箱の下に「9は範囲外、ここで止まる」。
- 下：枠「表 `books`」の中に行カード（`.row`）を5枚「本4」「本5」「本6」「本7」「本8」。二つの箱の ✓ の下から表の枠へ、`.ref` の矢印を1本。ラベル「③ 行を取り出す」。
- 表の枠の下（または右）に数のまとめを3つ：「探す 1回」「開く箱 2個」「取り出す行 5行」。「取り出す行 5行」を焦点の枠（`.hot`）にする。
