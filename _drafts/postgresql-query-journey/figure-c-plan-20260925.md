...
pairs = [
    ...
    ('*本全体の道のり。各章で、この中の仕組みを一つずつ確かめます。*',
     '*箱に付けた章番号が、その仕組みを確かめる章です。どの仕組みも、第1章のEXPLAINを使って調べます。*'),
]
...
計画の文字列（Task 1 のキャプション、Task 10 の図の直前の文）は、下の直しの後の本文に合わせてある。

| 場面 | 図・道具 | 計画のままで出た問題 | 直し方 |
| --- | --- | --- | --- |
| 実装担当の報告 | `00-book-sketch` | 代替テキストに第10〜11章の段が抜けていた。カードの中の幅144に、数字カード6枚と「→」が横に入らなかった | 代替テキストに「計画の選び方と更新・保守」を入れた。数字カードは左右2列に3枚ずつ積んだ |
| タスクのレビュー | `00-books-and-records` | `JOIN` のラベルが books の表のすぐ下にあり、表の注記に見えた | ラベルを下げ、対応の線へ引き出し線を伸ばした |
| タスクのレビュー | `00-ranking-question` | 焦点の枠が矢じりの端から15しか離れていなかった（決まりは20） | 三つの「？」と焦点の枠を右へ15ずらした |
| タスクのレビュー | `06-btree-path` | 枝と矢印の起点 y=168 が案内板の枠線に1.5重なった | 起点を枠の外側の y=169.5 にした（`06-range-scan` もこの座標を写した） |
| タスクのレビュー | `06-range-scan`（計画） | 「① 4がある箱へ」は扇形の枝の間に入らず、「② 隣の箱へ」は箱のすき間に入らなかった | 「① 4の箱へ」を真ん中の矢印の左に置き、②は箱の上に置いて引き出し線でつないだ |
| タスクのレビュー | `06-range-scan`（計画） | 「③ 行を取り出す」は②の後の手順に見え、箱をたどり終えてから表を読むと読めた（本文132行目と、Index Scan が1行ずつ受け渡すことに食い違う） | 表への矢印のラベルから番号を外した |
| タスクのレビュー | `03-storage-question` | 図の直前の文がファイルの問いで、ファイルを描かない図と合わなかった | 図とキャプションを、100万行と7,353回を述べた段落の直後へ移した（文は変えていない） |
| タスクのレビュー | `03-ctid-location` | 「ページ番号」だけ線がなく、上の `id` の1のラベルとも読めた | 「項目番号」と左右対称の線を付け、①の矢印をその角から始めた |
| タスクのレビュー | `03-index-reference` | 図の直前の文から「経路」の語が消え、直後の「この経路なら」の指す先が分かりにくかった | 元の文の言い回しを生かし、「…Index Scanの経路を、図で比べてみましょう」にした |
| 全体レビュー | `06-range-scan`・設計書4.3 | 範囲外の9の×が、開かない箱と同じ灰色の線の×だった。読まない物の描き方が2通りあり、使い分けが書かれていなかった | 9は紺の文字の×にし、4.3に描き分け（多数のページ・開かない物・比べて外れた値）を書いた |
| 全体レビュー | 問いの箱（M17） | 問いの箱の地と焦点の地が目では区別できない。問いの箱が部品見本にも設計書にもなかった | 色は変えず、線の色と太さと位置で見分けると決め、部品見本と4.1に書いた |
| 全体レビュー | 設計書9章・画像の README・序章のキャプション | PR #19 が未マージなのに「直した」と読めた。README の冒頭が試作のころのまま。序章の本文とキャプションが続けて「見てください」で終わった | 未マージと書き、README を31枚に合わせ、キャプションを言い換えた |
| 全体の一覧 | catalog | 第5・11・12章の並びが本文の順でなかった | 各章の図を本文に出てくる順にした |

横展開の次の段階に活かすこと：

- 同じ図形を写す2枚（第3章の木など）は、先の図を承認してから後の図を始める。後の図のラベルの長さと置き場所は、先の図の座標で測ってから依頼書に書く。
- ①②③の番号は「その順に起きる」と読まれる。実際には重なって起きる処理（Index の探索と表からの取り出しなど）には、番号を付けない。
- 図の直前の文と図の問いが合わないときは、文を変える前に、図の位置を動かして合わないかを考える。
- 問いの箱と焦点の塗りは同じ薄い橙のまま、線の色と太さと位置で見分ける（設計書4.1）。段階D・Eの問いの図7枚も同じ書き方にする。
