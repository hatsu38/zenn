...
- これはエージェントによる再実測。2026-09-24のユーザー提供の3回（すべてhit=8）は履歴として保持し、本文の掲載結果はこの再実測に置き換えた。準備SQLの出力の出典も同じログに揃えた（内容は2026-09-24のユーザー提供分と同一）。本文に掲載した追い出しのSQL2文の間には、ログでは`pg_buffercache`の件数確認（追い出し前11、追い出し後0）が挟まっている。この確認は`books`のバッファを変えないので、掲載時は省いた。

## 第4章のページごとの行数（2026-09-25）

図 `03-pages-and-rows` と `03-row-width` の出典。実験用ComposeのPostgreSQL 18.6（検証専用DB `journey_million_20260923`）で、第4章と同じ手順で観察用の表 `book_observation.books`（本1〜1000）を作り、`ctid` のページ番号ごとに行数を数えた。行幅実験の `size_short`・`size_long` も同じトランザクションの中で作って数えた。SQLは `chapter04-rows-per-page-20260925.sql`、出力は `chapter04-rows-per-page-20260925.log`。

- 観察用の表：表本体65,536バイト＝8ページ。ページ0〜6に136行ずつ（本1〜952）、ページ7に48行（本953〜1000）。本5の`ctid`は(0,5)。
- `size_short`（10文字）：65,536バイト＝8ページ。行が入っているのはページ0〜5（185行×5＋75行）で、ページ6・7は空き。
