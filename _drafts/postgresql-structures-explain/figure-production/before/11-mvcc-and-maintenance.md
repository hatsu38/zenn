...
```mermaid
sequenceDiagram
    participant A as 接続A
    participant D as データベース
    participant B as 接続B
    A->>D: Repeatable Readで検索
    D-->>A: 元の題名
    B->>D: 題名を変更してCOMMIT
    A->>D: 同じトランザクションで再検索
    D-->>A: 元の題名
...
```mermaid
flowchart TB
    K["同じ本：id 42"] --> O["古い版：実験用の本 42"]
    K --> N["新しい版：改訂版の本 42"]
    A["古い時点を読むA"] --> O
    B["新しい検索"] --> N
```

これは論理的な模型です。実際のページ配置や索引の更新をすべて描いたものではありません。
...
```mermaid
flowchart TB
    A["古い行バージョン"] --> B{"まだ必要な読み手がいる？"}
    B -->|"いる"| C["今は回収できない"]
    B -->|"いない"| D["VACUUMで領域を再利用可能にする"]
```
