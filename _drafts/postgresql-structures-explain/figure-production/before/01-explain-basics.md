...
実行時間は環境や実行のたびに変わります。まずは`rows=1.00`と`Rows Removed by Filter: 999`を見てください。

`actual`は「実際の」という意味です。`actual ... rows`と`Rows Removed by Filter`を探してください。後者は、条件に合わず除外した行です。

次は、この初期データを単純な1回の逐次走査で読んだ場合の**行数の説明図**です。時間の実測結果ではありません。

```mermaid
flowchart TB
    A["本の一覧：1,000行"] --> B["題名を1行ずつ比べる"]
    B -->|"一致：1行"| C["結果として返す"]
    B -->|"不一致：999行"| D["結果から除く"]
```
...
```mermaid
flowchart TB
    A["番号用の索引で42の場所を探す"] --> B["表から目的の行を読む"]
    B --> C["結果：1冊"]
```
