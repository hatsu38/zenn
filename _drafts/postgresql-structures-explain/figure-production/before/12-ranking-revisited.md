...
まず、次の図の各段に自分の計画の行数を書き込んでください。図は仕事を整理する模型で、実行計画のノードがいつも同じ並びになるという保証ではありません。

```mermaid
flowchart TB
    R["期間内の読了記録を取り出す"] --> J["本の情報と対応させる"]
    B["本の一覧"] --> J
    J --> A["本ごとに数える"]
    A --> S["件数順に上位を選ぶ"]
    S --> L["20冊の結果"]
...
行の流れを描くと、題名を付ける位置が変わります。

```mermaid
flowchart TB
    R["期間内の読了記録"] --> A["本の番号ごとに数える"]
    A --> S["上位20冊の番号を決める"]
    S --> J["20冊に題名を付ける"]
    B["本の索引"] --> J
    J --> O["20冊の結果"]
...
```mermaid
flowchart TB
    A["読了記録が増える・訂正される"] --> B["どこかの時点で集計を更新"]
    B --> C["保存した集計結果"]
    C --> D["画面は集計結果から20冊を読む"]
```
