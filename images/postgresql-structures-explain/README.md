...
序章から第12章まで、計44枚を本文へ掲載しています。序章は導入の絵2枚と技術図2枚、その他の章は技術図2〜3枚と場面の絵1枚です。[図の一覧](index.html)からまとめて確認できます。

| 章 | 題材 | SVG原本 | 掲載用PNG |
| --- | --- | --- | --- |
| 序章 | 本全体のグラレコ | [SVG](sources/00-book-sketch.svg) | [PNG](00-book-sketch.png) |
| 序章 | 読書記録サービスの画面イメージ | [SVG](sources/00-reading-service.svg) | [PNG](00-reading-service.png) |
...
| 第1章 | 結果が1行でも、調べたのは1,000行 | [SVG](sources/01-scan-and-filter.svg) | [PNG](01-scan-and-filter.png) |
| 第2章 | 接続ごとにプロセスが動く | [SVG](sources/02-connections.svg) | [PNG](02-connections.png) |
| 第2章 | 計画の木を、行が上へ進む | [SVG](sources/02-execution-tree.svg) | [PNG](02-execution-tree.png) |
| 第3章 | 索引から、表の行へたどる | [SVG](sources/03-index-reference.svg) | [PNG](03-index-reference.png) |
| 第3章 | 表の中にページ、ページの中に行 | [SVG](sources/03-pages-and-rows.svg) | [PNG](03-pages-and-rows.png) |
| 第3章 | 同じ1,000行でも、8ページと32ページ | [SVG](sources/03-row-width.svg) | [PNG](03-row-width.png) |
| 第4章 | ページの保存場所と、計算の作業場所 | [SVG](sources/04-memory-regions.svg) | [PNG](04-memory-regions.png) |
| 第4章 | 同じページを、別の接続でも使える | [SVG](sources/shared-buffer-reuse.svg) | [PNG](shared-buffer-reuse.png) |
| 第5章 | LIMIT 1でも、探す量は変わる | [SVG](sources/05-limit-search.svg) | [PNG](05-limit-search.png) |
| 第5章 | 見つけても、そこで終わりとは限らない | [SVG](sources/05-linear-scan.svg) | [PNG](05-linear-scan.png) |
| 第6章 | 境界を比べて、読むページを絞る | [SVG](sources/06-btree-path.svg) | [PNG](06-btree-path.png) |
| 第6章 | 4以上8以下なら、隣の葉も読む | [SVG](sources/06-range-scan.svg) | [PNG](06-range-scan.png) |
...
| 第2章 | 画面の向こうで、SQLが動く | [SVG](sources/02-client-scene.svg) | [PNG](02-client-scene.png) |
| 第3章 | 本の一覧は、どこに保存される？ | [SVG](sources/03-storage-question.svg) | [PNG](03-storage-question.png) |
| 第4章 | 同じ検索を、もう一度 | [SVG](sources/04-repeat-observation.svg) | [PNG](04-repeat-observation.png) |
| 第5章 | 本が増えても、探すのは同じ1冊 | [SVG](sources/05-growing-library.svg) | [PNG](05-growing-library.png) |
| 第6章 | 目録も大きい。それでも速い？ | [SVG](sources/06-catalog-question.svg) | [PNG](06-catalog-question.png) |
...
図の設計意図・用語と模型の対応は [制作方針](../../books/postgresql-structures-explain/ILLUSTRATION-GUIDE.md) にあります。

## PNGを再出力する

Node.jsがある環境で、リポジトリのルートから初回のみ準備します。

```sh
cd scripts/book-figures
npm install
npx playwright install chromium
npm run export
```

次回以降は、リポジトリのルートで以下を実行します。

```sh
npm --prefix scripts/book-figures run export
```

`sources/`内のSVGをすべてChromiumで描画し、フォントの読み込み後に2倍解像度でPNGを書き出します。出力先はこのREADMEと同じディレクトリです。同名のPNGは更新されます。元のSVGは変更しません。

PNGは横1280ピクセルで、高さは図ごとに異なります。共有バッファの3コマは1280×3060、ヒープの4コマは1280×3580ピクセルです。本文では横幅に合わせて縮小されます。文字やコマの配置を変えたら、幅360pxの表示も確認してください。

## 本文に掲載する

```md
![絵を見なくても変化が伝わる代替テキスト](/images/postgresql-structures-explain/shared-buffer-reuse.png)
