...
PR の変更概要や動作確認の結果も、Markdown のコメントで頑張るより、HTML にして見せるほうが断然伝わります。
レビュアーは URL を1クリックするだけで、diff の解説や、ログインから操作完了までのスクショを順に追えるようになる。この体験はかなり良いです。
で、その HTML の置き場と共有経路には、zip 化不要になった Artifact がちょうどよかったんですよね。
リポジトリの権限で守られた URL が発行されて、PR に貼れて、期限が来たら勝手に消える。
まずは動作確認のスクショを HTML 1枚にまとめて PR に貼るところから、ぜひ試してみてください。
