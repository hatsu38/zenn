...
![SQL は 3 つのフェーズを通る。EXPLAIN はそのうち「プラン」を覗き見るコマンド](/images/cddb89f9abfaca/03-sql-three-phases.png)
...
psql で接続するには、ホストに `psql` が入っているならそのまま:

```bash
psql -h localhost -p 5433 -U postgres -d explain_sandbox
```

ホストに `psql` が入っていない場合はコンテナ経由で:
...
:::message
**ポート 5433 を使う理由**: 多くの開発機ではホスト側の 5432 を別の Postgres（Homebrew や他プロジェクト）が使っています。衝突を避けるために 5433 にマップしてあります。
:::
...
![ヒープページ俯瞰と 1 ページ拡大。約 9 タプル × 11,181 ページ ≈ 100,000 行](/images/cddb89f9abfaca/02-heap-page-tuple-structure.png)
...
![Seq Scan のコスト構造。青の I/O コスト (11,181) と緑の CPU コスト (1,000) の足し算で 12,181.00](/images/cddb89f9abfaca/01-seq-scan-cost-structure.png)
