...
| テーブル | 文字列の長さ | 行数 | テーブル本体の大きさ | ページ数 |
| --- | ---: | ---: | ---: | ---: |
| `size_short` | 10文字 | 1,000行 | 65,536バイト | 8ページ |
| `size_long` | 200文字 | 1,000行 | 262,144バイト | 32ページ |

ページの大きさは、どちらのテーブルでも8,192バイトです。同じ大きさのページに、短い行と長い行を入れる様子を比べてください。

![短い行は6ページに最大185行ずつ、長い行は30ページに最大34行ずつ入り、どちらのテーブルにも行の入っていないページが2つある](/images/postgresql-query-journey/03-row-width.png)
*1ページに入る行の数（最大185行と34行）の差が、ページ数の差になっています。破線は行の入っていない空きページです（2026年9月25日の実測）。*

### 行数だけでは、読む量を比べられない

行が実際に入っているのは6ページと30ページで、この部分は5倍です。一方、ファイルには行がまだ入っていないページも2枚ずつ含まれ、テーブル本体のサイズは8ページと32ページ、**4倍**になりました。PostgreSQLはファイルの拡張時に複数ページをまとめて確保することがあり、行数から必要最小限のページ数を計算してもファイルのサイズとは一致しない場合があります。長い行は1ページに入る数が少なくなるので、より多くのページが必要です。

テーブル全体を読むなら、長い行のテーブルでは4倍のページを読むことになります。「どちらも1,000行だから、読むデータ量も同じ」とは言えません。ただし、この実験で測ったのは保存サイズです。読み取り時間が4倍になるかどうかは、まだ測っていません。
