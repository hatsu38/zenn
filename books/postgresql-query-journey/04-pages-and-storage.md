...
元の実測では独立した`BEGIN`〜`ROLLBACK`で測っています。新構成は外側の観察用トランザクションを保つためSAVEPOINTを使うので、制御コマンドの完了表示は異なります。サイズの値は参考例です。
...
元の実測では最後の`ROLLBACK`で表の作成を取り消しました。今回の手順では`ROLLBACK TO SAVEPOINT row_width`で、この行幅実験の表だけを取り消します。`size_short`と`size_long`は残らず、元の`books`にも変更は加えていません。
