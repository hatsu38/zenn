...
第8章で取り出した最新20件には、本の番号`book_id`はありますが、題名はありません。一覧に題名を出すには、記録にある番号からbooksテーブルをたどり、対応する行を組み合わせます。この対応付けが**結合（JOIN）**です。

![読了記録には本番号2・1・2しかなく、題名は画面の外のbooksテーブルで番号の行を探して付ける](/images/postgresql-query-journey/09-title-lookup.png)
