...
前章の計画の木に出てきた`Sort`を、今度はサービスの画面で使います。作るのは「最近読み終えた記録の一覧」です。記録が入っている順と、日時の新しい順は同じとは限りません。
...
この章のSortは、対象週の499,998行すべてに順位を付けました。ところが、最近の記録の画面に表示するのは新しい20件だけです。残りの約50万行の順位まで決める必要はあるのでしょうか。次章では`LIMIT 20`を付けて、並べ替えの仕事がどう変わるかを調べます。
