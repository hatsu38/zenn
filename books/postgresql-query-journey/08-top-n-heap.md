...
`LIMIT 20`を外すと、対象週のすべての行が必要になり、索引を途中で読み終えられません。索引を順に読む処理や、必要なら表の行を取り出す処理が増えるので、選ばれる方法も変わりえます。`LIMIT 20`を付けたときと、計画、行数、BUFFERSを比べましょう。
...
索引を作った後の計画でSortが消えていれば、最新20件は索引の順に取り出せています。ただし、この20件には本の番号しかなく、画面に題名を出せません。「今週よく読まれた20冊」を作るには、記録に題名を付ける処理と、同じ本の記録を数える処理も必要です。次章では、まずこの最新20件に題名を付け、続いて本ごとに数えます。今回の索引は残して進みます。
