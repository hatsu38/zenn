...
*各コマの上の▼が、いま届いたカードです。根の最小値と比べて、入れ替えるかを決めます（説明するための図。カードは行に対応します）。*
...
ここでは`Heap Fetches: 20`もあります。20行を返すためにテーブルの行も確認しました。Index Only Scanでもテーブルへの確認がありうる理由は、第11章で確かめます。
...
`LIMIT 20`を外すと、対象週のすべての行が必要になり、Indexを途中で読み終えられません。Indexを順に読む処理や、必要ならテーブルの行を取り出す処理が増えるので、選ばれる方法も変わりえます。
