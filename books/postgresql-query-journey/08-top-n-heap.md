...
`LIMIT 20`を外すと、対象週のすべての行が必要になり、索引を途中で読み終えられません。索引を順に読む処理や、必要なら表の行を取り出す処理が増えるので、選ばれる方法も変わりえます。`LIMIT 20`を付けたときと、計画、行数、BUFFERSを比べましょう。ここでも結果は載せないので、自分の出力で確かめてください。
