...
`n`は全入力、`k`は残したい件数です。1枚を入れ替えるたびに、根から木の高さの分だけ値を交換します。木の高さは候補数`k`に対して`log k`程度なので、`n`枚を確かめる比較の回数はおよそ`n log k`になります。これを`O(n log k)`と書きます。入力の件数`n`は減らせず、残す件数`k`が小さいほど1回の入れ替えが軽くなります。

実行計画に`top-N heapsort`が出ていたら、PostgreSQLは上の3枚のゲームと同じく、上位`k`件の候補だけをヒープに保ちながら並べています。小さいメモリ表示でも、子のScanが多くの行を返していないか確認してください。

日時用の索引を作る前に確認したとき、Sortの行は次のとおりでした。条件は、PostgreSQL 18.6、読了記録200万件、対象週499,998行、`work_mem`は4MBです。

```sql
Sort Method: top-N heapsort  Memory: 26kB
```

ソートへ入ったのは499,998行、上で返すのは20行でした。26kBという小さな作業領域は、候補の20行分です。調べた行数は499,998行のままなので、この値だけを見て「20行しか調べなかった」とは読めません。
...
先ほどのSQLをもう一度測ってください。Sortが残っているか、どの索引を使ったか、何行読んだかを比べます。この結果は本書に載せていないので、自分の出力で確かめる課題です。
...
ここで用意したのは、WHEREの日時条件にも、ORDER BYの並び順にも合う索引です。索引の並びとは関係のない条件で多数の行が除かれるなら、20件を得るためにもっと多くの行を読むこともあります。

`Index Only Scan`という名前が出ても、「絶対に表を読まない」という意味にはしないでください。表への確認が必要かは第11章で扱います。

## たくさん欲しくなったら、もう一度比べる

`LIMIT 20`を外すと、対象週のすべての行が必要になり、索引を途中で読み終えることはできません。索引を順に読む処理や、必要なら表の行を取り出す処理が増えるので、選ばれる方法も変わりえます。`LIMIT 20`を付けたときと、計画、行数、BUFFERSを比べましょう。ここでも結果は載せないので、自分の出力で確かめてください。
...
日時順の最新20件は、索引を使って取り出せるようになりました。ただし、この20件には本の番号しかなく、画面に題名を出せません。「今週よく読まれた20冊」を作るには、記録に題名を付ける処理と、同じ本の記録を数える処理も必要です。次章では、まずこの最新20件に題名を付け、続いて本ごとに数えます。今回の索引は残して進みます。
