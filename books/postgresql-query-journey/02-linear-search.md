...
5冊に縮めて考えます。Seq Scanは、まだ読んでいない本の題名を知りません。条件に合う行をすべて求める検索で、本2が一致した後、どこまで題名を比べるかを見てください。

![5冊を先頭から順に比べ、2冊目の星が一致した後も3冊目から5冊目まで比べる。比べたのは5冊で、除外4冊と返した1冊](/images/postgresql-query-journey/05-linear-scan.png)
*5冊へ縮めて説明するための図。条件に合う行をすべて返す検索なので、一致した後も最後まで比べます。*
...
*Limitが次の行を求めなくなった時点で、Seq Scanも止まります（説明するための図）。*
...
返した行は0行、除外した行は100万行です。1行も見つからないので、テーブルを最後まで調べています。
...
この章の四つの検索で、返した行はどれも1行か0行でした。変わったのは調べた行数で、42行から100万行まで開きがあります。結果の件数を見ても、そのために何行を調べたかは分かりません。

上位20冊を返すランキングも同じです。結果が20行だから調べる記録も20件、とは限りません。どの本が20位以内に入るかは、対象の記録を数え終えるまで決まらないかもしれません。

「LIMITを付けたので大きなテーブルでも大丈夫」という提案を検証するときは、目的の行が後ろにある場合と、存在しない場合の`Rows Removed by Filter`も確かめます。

出力には`Buffers`の値も並んでいました。行はいくつかずつまとめた「ページ」という単位で保存されていて、`Buffers`はそのページへのアクセスを表します。読み方は第4章と第5章で確かめます。テーブルを最後まで調べる検索では、ページがメモリにあってもなくても、100万行に条件を当てる仕事はなくなりません。

第1章で同じ本を`WHERE id = 42`で探したときは、`Index Scan`になり、`Rows Removed by Filter`は表示されませんでした。番号では、途中の行を一つずつ確認せずに探せていたことになります。次章では、探す範囲を狭めるIndexの仕組みを見て、題名でもこの探し方ができるかを確かめます。
