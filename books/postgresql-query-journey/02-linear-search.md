...
第1章で同じ本を`WHERE id = 42`で探したときは、Index Scanになり、`Rows Removed by Filter`は表示されませんでした。番号では、途中の行を一つずつ確認せずに探せていたことになります。次章では、探す範囲を狭める索引の仕組みを見て、題名でもこの探し方ができるかを確かめます。
