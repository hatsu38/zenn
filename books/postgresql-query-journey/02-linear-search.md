...
今回は、**41＋1＝42行を確認したところで終了**しています。今回`LIMIT 1`で変わったのは、探し方ではありません。必要な結果がそろった時点で止まるようになった、という変化です。
...
目的の行が見つかると、`Limit`は下の`Seq Scan`にそれ以上の行を求めずに済みます。今回は、`Limit`が1行を受け取った時点で走査が終わりました。
...
この表は、今回の単純なSeq Scan（`loops=1`）での観察です。SQLに`ORDER BY`がなければ、行が返る順序は決まっていません。本の番号の順に見つかるとも限らないので、自分の出力でも行数を確かめてください。
...
行は、いくつかずつまとめた**ページ**という単位で保存されています（第4章で調べます）。出力の`BUFFERS`はそのページへのアクセスを表しますが、読み方は第4章と第5章で確かめます。100万行の検索がメモリ内で済んでも、100万行に条件を当てる仕事はなくなりません。

上位20冊を返すランキングも、結果が20行だから調べる記録も20件、とは限りません。どの本が20位以内に入るかは、対象の記録を数え終えるまで決まらないかもしれません。

「LIMITを付けたので大きな表でも大丈夫」という提案を検証するときは、目的の行が後ろにある場合と、存在しない場合の`Rows Removed by Filter`も確かめます。

第1章では、同じ本を`WHERE id = 42`で探すと索引が使われ、`Buffers`は`shared hit=7`でした。次章では、途中を一つずつ確認せずに探す範囲を狭める索引の仕組みを見て、題名でもこの探し方ができるかを確かめます。
