...
*表の四つの実測を帯にしたもの。この章の設定で、毎回表の先頭から読んでいます。*
...
出力には`Buffers`の値も並んでいました。行はいくつかずつまとめた「ページ」という単位で保存されていて、`Buffers`はそのページへのアクセスを表します。読み方は第4章と第5章で確かめます。表を最後まで調べる検索では、ページがメモリにあってもなくても、100万行に条件を当てる仕事はなくなりません。
