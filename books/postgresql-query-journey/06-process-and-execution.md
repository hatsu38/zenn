...
前章では、データのページを共有バッファで再利用できることを学びました。ここでpsqlを二つ開いたら、何を共有し、何が別々になるでしょうか。接続の先で動いているプロセスを調べます。
...
共有するページと、処理ごとの作業領域を区別できました。次はサービスの「最近の読了記録」を作ります。記録を日時順に並べるとき、この作業領域をどのように使うか調べましょう。
