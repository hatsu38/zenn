...
次に、表を順に読むか、Indexを使うかなどを比較して、実行計画を作ります。実行計画を作る部分を**プランナ**と呼びます。出来上がった計画に従ってページを使い、条件を確かめて行を返す部分が**エグゼキュータ（実行器）**です。[公式の処理経路の説明](https://www.postgresql.org/docs/18/query-path.html)も、この順序で整理されています。
...
今回の計画には、並べ替えを表す`Sort`がありません。題名のIndexから、`ORDER BY title`で求めた順に行を取り出せるためです。Indexで順序を得られない場合の並べ替えは、次章で扱います。
...
psqlから送ったSQLは、その接続のバックエンドが受け取り、実行計画を作って、必要なページを共有バッファから使います。二つの接続は別々のバックエンドを持ちながら、同じ表やIndexのページを共有バッファから使えます。

今回の計画では、`Index Scan`から`Limit`へ題名順に行が渡され、3行で要求を止める予定でした。では、読了記録を日時順に並べたいとき、その順序のIndexがなかったらどうなるでしょうか。次章では`Sort`を観察し、並べ替えの処理と作業用メモリの使い方を確かめます。
