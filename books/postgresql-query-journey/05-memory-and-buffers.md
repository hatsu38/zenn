...
PostgreSQLは、読み込んだ表や索引のページをメモリ内に保持します。この領域は、同じDBに接続した複数の接続から使えるので、**共有バッファ**と呼びます。次の検索でも必要なページが残っていれば、ファイルから読み込み直さずに使えます。
...
この区別で、これまでの出力を振り返れます。第1章の題名検索は`shared hit=5112 read=2241`、第2章の同じ検索は`hit=5537 read=1816`、第3章で索引を作る前の検索は`hit=7353`で`read`がありませんでした。同じ表を読み返すうちに、ファイルから読み込んだページが共有バッファに残り、`read`が`hit`に置き換わっていった例です。ただし、この間には別のSQLも実行しているので、厳密な比較実験ではありません。
...
3回とも、検索の実行中の表示は`shared hit=8`でした。共有バッファにあるページを8回使っていて、`shared read`は表示されていません。先に述べたとおり、この比較は最初からページがメモリにある状態で始まっています。
