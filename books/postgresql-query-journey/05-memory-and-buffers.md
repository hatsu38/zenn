...
図の上側の「8、3、5」は、接続Aと接続Bがそれぞれ並べ替えている途中の値です。並べ替えは**ソート**とも呼びます。接続Aの途中の値を接続Bが使うことはないので、作業領域は接続ごとに分かれています。一方、下側の共有バッファにあるページP0〜P3は、どちらの接続からも使えます。この章の検索が2回目から`hit`になったのは、この下側の領域にページが残っていたからです。
...
| `work_mem` | ソートなど、一つの処理が作業に使うメモリ量の目安 |
...
この出力は設定値であり、いま各処理が実際に使っているメモリ量ではありません。たとえば`work_mem`の`4MB`を見ても、「この検索で4MB使った」とは読めません。この章の題名検索には並べ替えがないので、`work_mem`が関わる処理は計画に出てきていません。
...
この章の図では、共有バッファは接続Aと接続Bの両方から使える一つの領域で、作業領域は接続ごとに分かれていました。何が共有され、何が接続ごとに分かれるのかを確かめるには、接続の向こう側で実際に動いているものを知る必要があります。psqlはSQLを送る側で、受け取って実行しているのは別のプログラムです。次章ではpsqlを二つ開き、接続ごとに動くプログラムと、共有するメモリの関係を確かめます。
