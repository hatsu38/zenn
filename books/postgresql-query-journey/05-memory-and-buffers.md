...
`buffers_flushed`の8は、追い出すときにファイルへ書き出されたページの数で、表本体の8ページがこれに当たります。挿入したばかりで、ファイルの内容よりメモリ上のページのほうが新しかったためです。この出力から、挿入した直後の表のページが共有バッファに置かれていたことも確かめられます。
...
SQLも、返る1行も、3回とも同じです。それなら前回の答えをそのまま返してもよさそうですが、PostgreSQLには、検索の答えそのものを取っておいて使い回す仕組みがありません。共有バッファに置くのはページだけです。この観察用の表は、トランザクションをまだ終えていないので他の接続からは見えませんが、ふだんの表なら、1回目と2回目の間に別の接続が同じ題名の本を追加したり、この本の題名を書き換えたりしているかもしれません。前回の1行を取っておいて返すと、その変更を見落とします。そのため、実行するたびにページの中の行を確認します。行の変更が他の接続からどう見えるかは、第11章で扱います。
...
こうした値を置くのが、処理ごとの**作業領域**です。図で、二つの接続がそれぞれ作業領域を持ち、共有バッファだけを共有している配置を見てください。
