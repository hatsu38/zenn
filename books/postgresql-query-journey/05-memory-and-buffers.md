...
:::message
第4章のトランザクションを終了してから始めます。この章も、観察用の表の作成からROLLBACKまで同じ接続で進めます。
:::
...
図で、3回分のページの使い方と、比べた行の数を見比べてください。
...
:::message
この結果は8ページの小さな表でのものです。大きな表のSeq Scanでは、共有バッファの一部を輪のように使い回す読み方を選ぶことがあります。PostgreSQL 18では、通常の共有バッファを使う表が共有バッファ全体の4分の1を超えることなどが、この読み方を選ぶ条件です。第4章の100万冊の表を続けて読んでも、2回目がすべてhitになるとは限りません。[実装の条件](https://github.com/postgres/postgres/blob/REL_18_STABLE/src/backend/access/heap/heapam.c)も参照できます。
:::
