...
SQLも、返る1行も、3回とも同じです。それなら前回の答えをそのまま返してもよさそうですが、PostgreSQLには、検索の答えそのものを取っておいて使い回す仕組みがありません。共有バッファに置くのはページだけです。この観察用の表はまだ他の接続から見えませんが、ふだんの表なら、1回目と2回目の間に別の接続が同じ題名の本を追加したり、この本の題名を書き換えたりしているかもしれません。前回の1行を取っておいて返すと、その変更を見落とします。そのため、実行するたびにページの中の行を確認します。行の変更が他の接続からどう見えるかは、第11章で扱います。
...
図の上側の「8、3、5」は、接続Aと接続Bがそれぞれ並べ替えている途中の値です。図では同じ数字にしていますが、それぞれが別々に持っている値です。並べ替えは**ソート**とも呼びます。接続Aの途中の値を接続Bが使うことはないので、作業領域は接続ごとに別々です。一つの接続の中でも、並べ替えなどの処理ごとに用意されます。一方、下側の共有バッファにあるページP0〜P3は、どちらの接続からも使えます。この章の検索が2回目から`hit`になったのは、この下側の領域にページが残っていたからです。
...
| `work_mem` | ソートなど一つの処理が、一時ファイルに書き出さずに使えるメモリ量の基準 |
...
この章の図では、接続Aと接続Bがそれぞれ自分の作業領域を持ち、共有バッファだけを一緒に使っていました。では、図の「接続A」「接続B」は、サーバの中では実際に何に当たるのでしょうか。次章ではpsqlを二つ開いて接続ごとに付いた番号を表示し、接続ごとに動くものと、共有するメモリの関係を確かめます。
