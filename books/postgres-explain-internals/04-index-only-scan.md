...
![UPDATE が起きると、古い行（薄黄）に削除マーク `xmax` が刻まれ、新しい行（薄黄）が同じテーブルに追加される。同じ `id` の行が 2 つ並んだ状態が一時的に生まれる](/images/postgres-explain-internals/ch04/02-mvcc-update-sequence.png)
...
![トランザクション ID を時間軸（左→右）に取り、各行の生存期間を帯で表す。T120 から縦に線を引いて、R1 の帯は線をまたぐので可視、R2 の帯は線より前で終わるので不可視、R3 の帯は線より後で始まるので不可視、というのが視覚的に見える](/images/postgres-explain-internals/ch04/03-visibility-from-t120.png)
...
![visibility map のライフサイクル。初期はビットが立っていない（=不明）。VACUUM が走ると all-visible に立つ。INSERT/UPDATE/DELETE が起きるとそのページのビットがクリアされる。autovacuum が再び立て直す、というループが繰り返される](/images/postgres-explain-internals/ch04/04-visibility-map-lifecycle.png)
