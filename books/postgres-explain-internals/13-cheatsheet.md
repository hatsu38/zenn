...
![EXPLAIN プラン一覧チートシート。本書で扱う全プランの一覧表（黄=Scan / 青=Sort / 緑=Join）](/images/postgres-explain-internals/ch13/01-plan-cheatsheet.png)
...
この本を書き始めたきっかけは、冒頭に書いた「インデックスを貼ったのに遅い」の一件でした。当時は出力を雰囲気で眺めていて、直した修正がなぜ効いたのかを人に説明できませんでした。13 章ぶんを書ききってみて変わったのは、EXPLAIN の出力を前に身構える瞬間がほとんどなくなったことです。知らないノードが出てきても、cost はどこから来たのか、rows の推定はどこでずれたのか、と同じ順番で問いを立てれば、たいていの答えは出力の中に書いてあります。
