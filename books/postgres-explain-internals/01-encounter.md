...
<!-- ↓ AI が書いた叩き台。本人の声に書き直す前提。 -->

「インデックスは貼ってあるのに、このクエリだけ遅い」。私が何度も踏んできた状況です。EXPLAIN を打てば原因がわかると聞いて実行しても、返ってきた出力の意味が読み取れませんでした。Seq Scan と書いてあるのは読める。けれど、なぜ貼ったはずのインデックスが使われず、テーブルを丸ごと読む計画になっているのか。その理由は、出力のどこにも文章では書いてありません。

当時の私は、出力をざっと眺めて「たぶんこの数字が大きいから遅いんだろう」と雰囲気で当たりをつけていました。当たることもあれば外れることもあり、外れたときになぜ外れたのかを説明できない。これが一番こわいところでした。原因を自分の言葉で説明できなければ、直したつもりの修正が本当に効いたのかどうかも判断できないからです。

この本で目指すのは、その状況を終わらせることです。EXPLAIN の出力を 1 行ずつ、そこに並ぶ数字がどこから来たのかまで追えるようにして、「インデックスを貼ったのに遅い」の理由を、推測ではなく出力を根拠に説明できるようになる。そこまで行くには、cost の決まり方からページとヒープ、統計情報、VACUUM といった PostgreSQL の内部のしくみまで降りる必要があります。最初の一歩は拍子抜けするくらい小さく、WHERE も JOIN もない `SELECT * FROM articles` のたった 1 行を読み切るところから始めます。
...
![SQL は 3 つのフェーズを通る。EXPLAIN はそのうち「プラン」を覗き見るコマンド](/images/postgres-explain-internals/ch01/01-sql-three-phases.png)
...
![キャプション](/images/postgres-explain-internals/ch01/02-seq-scan-page-sweep.png)
...
第 1 章でわかったのは、EXPLAIN は「実行する前の計画書」であって、クエリそのものは動かさないということでした。そして `cost=0.00..12181.00` の `12181` は、秒でもミリ秒でもなく、ページ数と行数から電卓で再現できるただの足し算の答えでした。魔法に見えていた数字の出どころが 1 つわかると、残りの出力も同じように「どこかに理由があるはず」と思って読めるようになります。次章では、この推定のコストに実測の時間が加わる `EXPLAIN ANALYZE` を読みます。
